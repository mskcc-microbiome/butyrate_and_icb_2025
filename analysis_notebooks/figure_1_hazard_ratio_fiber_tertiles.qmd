...
#| warning: false
#| include: true
#| label: prep_data_for_km_curves

all_metadata_fiber <- all_metadata_fiber %>%
  mutate(fiber_tertile = ntile(aofib, 3)) %>%
  mutate(fiber_t_factor_ordered = factor(fiber_tertile, ordered = T, levels = c(1,2,3))) %>%
  mutate(fiber_top_tertile = fiber_tertile==3) %>%
  mutate(fiber_top_tertile_factor = factor(fiber_top_tertile))

tertiles <- quantile(all_metadata_fiber$aofib, probs = c(1/3, 2/3))
print(tertiles)

pval_continuous_pfs <- summary(coxph(Surv(pfs_mo, pod_status)~aofib,
                                     data=all_metadata_fiber))$coefficients["aofib", "Pr(>|z|)"] %>%
  signif(digits = 3)

pval_continuous_os <- summary(coxph(Surv(os_mo, os_status)~aofib,
                                    data=all_metadata_fiber))$coefficients["aofib", "Pr(>|z|)"] %>%
  signif(digits = 3)

pval_multivariate_binary_pfs_top_bottom_tertile <- summary(coxph(Surv(pfs_mo, pod_status)~fiber_top_tertile_factor + 
                            ageby10 + 
                            ecog_1to5_final +
                              cancer_RCCvsUC+
                              stage + 
                              bmi,
                                     data=all_metadata_fiber, subset = (fiber_tertile %in% c(1, 3))))$coefficients["fiber_top_tertile_factorTRUE", "Pr(>|z|)"] %>%
  signif(digits = 3)
print(pval_multivariate_binary_pfs_top_bottom_tertile)

pval_multivariate_binary_os_top_bottom_tertile <- summary(coxph(Surv(os_mo, os_status)~fiber_top_tertile_factor + 
                            ageby10 + 
                            ecog_1to5_final +
                              cancer_RCCvsUC +
                              stage +
                              bmi,
                                     data=all_metadata_fiber, subset = (fiber_tertile %in% c(1, 3))))$coefficients["fiber_top_tertile_factorTRUE", "Pr(>|z|)"] %>%
  signif(digits = 3)
print(pval_multivariate_binary_os_top_bottom_tertile)
...
kmPFS_tert<-survfit(Surv(pfs_mo, pod_status)~fiber_t_factor_ordered,
               type="kaplan-meier", data=all_metadata_fiber)

pfs_plot <- ggsurvplot(kmPFS_tert,risk.table=FALSE,
           xlab=element_blank(), 
           ylab=element_blank(),
           pval = pval_continuous_pfs,
           palette = custom_palette,
           title=element_blank(),
           #legend = "none",
           legend.labs=c("0-33%", "33-66%", "66-100%"), # an ordered list
           legend.title = "Fiber Consumption Tertile",
           conf.int=F,
           break.x.by=6,
           axes.offset=FALSE,
           xlim=c(0,25),
...
kmOS_tert<-survfit(Surv(os_mo, os_status)~fiber_t_factor_ordered,
               type="kaplan-meier", data=all_metadata_fiber)


os_plot <- ggsurvplot(kmOS_tert,risk.table=F,
           xlab=element_blank(), 
           ylab=element_blank(),
           size = .5,
           pval = pval_continuous_os,
           palette = custom_palette,
           #title=element_blank(),
           #legend = "none",
           legend.labs=c("0-33%", "33-66%", "66-100%"), # an ordered list
