...
ggplot(external_rcc, aes(x = response, y = pyruvate)) +
  ...
  geom_jitter(aes(fill = response), shape = 21, width = 0.15, size = 2, stroke = 0.3, color = "black", alpha = 0.7) +
...
ggsave("svg/updated_fig_2_d.svg", w = 1.5, h = 2.5) 
...
results_df$modelname = factor(results_df$modelname, levels = c("log(F prausnitizii + 1e-7)", "log(Clostridia)","log(Clostridiales)",
                                                               "log(Ruminococcacaeae + Lachnospiraceae)", "log(Ruminococcaceae)", "log(ACoA)" ))

ggplot(results_df) +
  geom_point(aes(y=modelname, x=HR)) +
  geom_errorbarh(aes(y=modelname, xmin=Lower_CI, xmax=Upper_CI), height=0.5)+
...
ggsave("svg/updated_fig_2_e_pt1.svg", w = 5.4, h = 2) 
```
...
ggplot(results_df) +
  ...
  labs(title = "HR (95% CI) multivariable Cox PFS\nMixed Solid Tumor Cohort",
       subtitle ="", y="Variable", x="HR")
ggsave("svg/updated_fig_2_e_pt2.svg", w = 5.4, h = 2) 
...
ggplot(results_df) +
  ...
  labs(title = "HR (95% CI) multivariable Cox OS\nMixed Solid Tumor Cohort",
       subtitle ="", y="Variable", x="HR")
ggsave("svg/updated_fig_2_e_pt3.svg", w = 5.4, h = 2) 
