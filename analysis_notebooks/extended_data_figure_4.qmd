...
cox_mst_os_mv_spline <-  coxph(Surv(tt_os_d, event_os)~rms::rcs(total,4)+age10+factor(cohort)+ecog + bmi + imputed_smoking_status,, data = mixed_solid_tumor)


summary(cox_mst_os_mv_spline)
ptemp <- termplot(cox_mst_os_mv_spline, se=T, plot=F)
buterm <- ptemp$total
...
ggplot(spline_data, aes(x = buty)) +
  ...
  theme_minimal() +
 theme(
     axis.text = element_text(size = 12), # Adjust axis labels
     axis.title = element_text(size = 12), # Adjust axis titles
     plot.title = element_text(size = 12),  # Adjust plot title
     legend.text = element_text(size = 9),  # Adjust legend text
     legend.title = element_text(size = 10), # Adjust legend titleMetronidazole
     strip.text.x = element_text(size = 10),
     strip.text.y = element_text(size = 10)
 ) 
ggsave("svg/updated_extended_figure_4_part_a_HR_tot_butyrate.svg", w = 3.5, h = 2)  
```
### Panel B
...
forest_os <- rbind(
univ2(coxph(Surv(tt_os_d, event_os)~log(total)+age10+cohort+ecog+ bmi + imputed_smoking_status, mixed_solid_tumor)),
univ2(coxph(Surv(tt_os_d, event_os)~log(pyruvate)+age10+cohort+ecog+ bmi + imputed_smoking_status, mixed_solid_tumor)),
univ2(coxph(Surv(tt_os_d, event_os)~log(lysine+0.0000001)+age10+cohort+ecog+ bmi + imputed_smoking_status, mixed_solid_tumor)),
univ2(coxph(Surv(tt_os_d, event_os)~log(fouraminobutyrate+0.000001)+age10+cohort+ecog+ bmi + imputed_smoking_status, mixed_solid_tumor)),
univ2(coxph(Surv(tt_os_d, event_os)~log(gluatarate+0.000001)+age10+cohort+ecog+ bmi + imputed_smoking_status, mixed_solid_tumor)))
...
forest_os %>%
 ...
 theme(
     ...
     plot.title = element_text(size = 10),  # Adjust plot title
     legend.text = element_text(size = 9),  # Adjust legend text
     legend.title = element_text(size = 9), # Adjust legend titleMetronidazole
     strip.text.x = element_text(size = 9),
     strip.text.y = element_text(size = 9)
 ) + 
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("svg/updated_extended_figure_4_part_b_forest_plt.svg", w = 4, h = 2)  
```
